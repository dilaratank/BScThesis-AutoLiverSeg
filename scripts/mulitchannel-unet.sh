#!/bin/sh
#SBATCH --job-name=mul1liv
#SBATCH --mem=75G               # max memory per node
#SBATCH --cpus-per-task=2      # max CPU cores per MPI process, error if wrong
#SBATCH --time=00-20:00        # time limit (DD-HH:MM)
###SBATCH --gres=mps:12   # dit script heeft een volledige GPU nodig, dus gebruik geen mps
...
echo "Starting"
nice -10 python3 train.py --arch 'multichannel-unet' --name 'multichannel-unet liver1'
echo "Done"
